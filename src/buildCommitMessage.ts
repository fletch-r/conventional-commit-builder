...
import commonCommitQuickPick from "./steps/common_commits/commonCommitQuickPick";
...
export default async function buildCommitMessage(
  ...
  const workspace_common_commits = config.get("commonCommits");
  ...
  let message_values: CommitMessageType = {
    type: "",
    scope: "",
    emoji: "",
    reference: "",
  ...
  let chosen_common_commit;
  if (workspace_common_commits.length > 0) {
  ...
  if (chosen_common_commit === undefined) {
    ...
    for (let current_step = 0; current_step < step_order.length; ) {
      ...
      switch (step) {
        case "<type>":
          await typeQuickPick(message_values.type).then((value: string) => {
